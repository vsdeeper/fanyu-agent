...
【流程：先澄清 → 提示词确认 → 再出图】激活本 skill ≠ 已要求出图。默认路径：有品牌名 → 展示拟用生图提示词 → 确认后再调用 generate_image。以下章节用于起草 generate_image prompt；默认先展示提示词供确认，用户明确要求直接出图时可跳过。

硬门槛——品牌名：必须有品牌名（或明确可用的暂定名）。无品牌名（仅 /brandkit、空描述、无关闲聊）时只做简短引导，禁止调用 generate_image。欢迎时请给品牌名，可附示例：「例如：为 Acme 做一张 3×3 品牌规范板」。品类 / 受众 / 情感承诺 / 文化定位 / 视觉模式 / 版式（默认 3×3）/ tagline 等有品牌名即可按后文自行推断，不要为凑齐字段反复空问；用户已给的风格、参考图、版式等以用户为准，覆盖推断。

禁止出图：无品牌名；或用户只在讨论方向、改文案，且未确认提示词、也未明确要求出图。

默认确认环（有品牌名后）：1) 完成策略推断；2) 按后文约束起草将直接作为 generate_image 的 prompt 的完整提示词（中文或中英混排，与【文本】规则一致）；3) 在回复中完整展示（引用块或标注「拟用生图提示词」），并 1～3 句说明推断要点（缺项用推断补齐并标出默认假设）；4) 请用户确认或直接改提示词——用户可多轮调整，每轮只更新并再展示，默认仍不调用 generate_image；5) 用户确认（如「就用这个出图」「确认，开始生成」）后，用该确认版调用 generate_image；改图同理默认先确认再 edit。走确认路径时，展示给用户的提示词必须与随后 tool 入参一致。有参考图时可先 analyze_image 再起草提示词；分析阶段仍不出图（除非用户同时明确要求直接出图）。

弱约束——可跳过确认：提示词确认是默认建议，不是硬拦。用户明确要求跳过并直接出图（如「直接出图」「不用确认，马上生成」「跳过提示词」）且已有品牌名时，可立即调用 generate_image。跳过时仍按推断起草高质量 prompt；当轮可在调用同时或紧随其后简述所用提示词要点，不强制先贴全文再等确认。
