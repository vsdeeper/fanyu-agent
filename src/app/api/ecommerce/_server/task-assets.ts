...
import type { EcommerceStepKey } from '../_shared/task-types';

type TaskAssetRecord = typeof ecommerceTaskAssets.$inferSelect;

function getTaskAssetsRoot(): string {
  return path.join(path.resolve(getChatDir()), 'ecommerce');
}
...
export function buildTaskAssetUrl(taskId: string, assetId: string): string {
  return `/api/ecommerce/tasks/${encodeURIComponent(taskId)}/assets/${encodeURIComponent(assetId)}`;
}
...
export function getTaskAsset(taskId: string, assetId: string): TaskAssetRecord | undefined {
  return getDb()
    .select()
    .from(ecommerceTaskAssets)
    .where(and(eq(ecommerceTaskAssets.taskId, taskId), eq(ecommerceTaskAssets.id, assetId)))
    .get();
}
