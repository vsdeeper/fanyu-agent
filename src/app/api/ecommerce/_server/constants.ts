...
export const PRODUCT_PLACEMENT_PROMPT_GUARD =
  '（服务端强制）产品必须稳定放置在场景中的支撑面（桌面、台面、地面、货架等）上，底座或支脚与支撑面真实贴合接触，接触处有自然、正确朝向的承重接触阴影；产品重心稳定、整体平衡，符合现实物理与透视；不得悬空、悬浮、漂浮，不得出现底座与支撑面分离、透视错位或腾空等违反物理的表现；场景中的墙面、地面、物件间的支撑关系须协调一致。若创意确需表现产品悬浮，必须在画面中给出明确、自洽的悬浮设定或悬浮载体（如透明悬浮台），否则一律贴合支撑面放置。';
