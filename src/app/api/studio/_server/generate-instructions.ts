...
export function buildImageTextPrompt(
  ...
  if (refs?.hasStyleReference) {
    if (requirement) {
      lines.push(
        '【视觉参考图】须对齐画风、画法媒介、配色、字体与整体气质（如线刻/版画/单色解剖图 vs 写实摄影，须与参考一致，禁止改成另一种渲染媒介）；人物肢体与皮肤的画法属于画风，须一并对齐；若参考为多人分栏，保留多人同框关系。',
        '反约束：勿复刻其原文案、图注、引出线与标记点落点；勿把参考图上的标识点、测量线或标注结构原样搬进成品；标识与标注位置以【我的要求】与【本张画面 / 图文内容】为准，参考图不作定位依据。',
        '版式密度以【我的要求】为准，勿因正文或参考图变成长文信息图。',
      );
    } else {
      lines.push(
        '【视觉参考图】须对齐画风、画法媒介、配色、字体与排版气质（如线刻/版画/单色解剖图 vs 写实摄影，须与参考一致，禁止改成另一种渲染媒介）；人物肢体与皮肤的画法属于画风，须一并对齐；若参考为多人分栏，保留多人同框关系。',
  ...
  if (requirement) {
    lines.push(
      '【我的要求】约束画面呈现（人物姿态、信息取舍、版式密度与板块结构），优先于正文默认的信息密度与章节结构：',
      requirement,
      '用户写明须保留或展示的板块、栏目、步骤、标注方式与版式密度，必须按要求落实；未写明的内容仍默认克制，勿自行扩成信息墙。',
      '姿态、角度、出镜方式的补充只改变人物动作，不得因此删减人物模特中的任一人物。',
      '若要求写明仅做标识标注、或不要展示其它内容：以人物为主体，只叠加必要标记点与引出标注；最多保留标题与一句说明；禁止多栏信息框、步骤列表、配图栏目、图标栏等长文信息图版式。',
      '【本张画面 / 图文内容】仅作事实与文案来源（知识库），非必须全文上屏；按【我的要求】提炼所需标题与文案；未要求铺开时，勿按正文 ## 小节结构整页铺开。',
      prompt.trim(),
    );
  } else {
    lines.push(
