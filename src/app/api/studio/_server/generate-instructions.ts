...
/** 长文配图：槽位画面描述 + 可选整套视觉约束，文生图。 */
export function buildLongArticleInlinePrompt(prompt: string, visualStyle?: string): string {
  const lines = [
    '生成恰好一张适合长文的配图，不要输出说明文字。',
    '画面干净、清爽且简洁、高级：构图上乘，留白是有意图的负空间而不是空荡，色彩克制，画面讲究质感与光影层次；信息层级清晰，避免杂乱文字墙；若提示词要求画面内文字，保持可读。',
    '禁止廉价感画风：扁平矢量插画、细描边涂鸦、简笔画、PPT 与素材库风的扁平小人、卡通表情风；简洁不等于简陋，不得用无细节色块与空画面充数。',
    '用摄影质感时须是布光讲究、背景干净、设计过的商业质感：禁止纪实、新闻、抓拍、生活随拍那种随手一拍的味道。',
    '若画面有人物：以中国人为主（东亚面孔、自然妆发与着装、中国大陆生活语境）；同一套配图人物形象须统一，禁止中西面孔或国内外气质混用。',
...
export function buildImageTextPrompt(
  ...
  const lines = [
    ...
    '文案上屏须克制：优先标题与少量关键句，字号够大、层级分明；宁可少写、写准，也不要为「内容完整」把要点、列表、步骤等一并塞进同一张图。',
    '禁止出现手机外框、刘海/挖孔、状态栏、浏览器顶栏、App 导航栏、桌面壁纸或任何设备/界面外壳；不要做成「手机截图」或「手机里的卡片」效果。',
  ];
  const requirement = refs?.characterRequirement?.trim();

  // 有「我的要求」时先注入并置顶，降低视觉参考姿态抢权重。
  if (requirement) {
    lines.push(
      '【我的要求】优先级最高，高于正文与视觉参考；下列原文必须逐条落实：',
      requirement,
      '姿态硬约束：写明的姿态、动作、手臂方向、手心朝向必须改到位，禁止照搬视觉参考图姿态。',
      '手臂方向：若写「向左伸 / 向右伸」，默认按画面左右（镜头所见），除非原文写明人物自身左右；须保持前臂大致水平侧伸，与要求方向一致。',
      '手心朝向释义（若要求含「掌心向前 / 掌心面向观众 / 朝向镜头」等）：在手臂侧伸前提下，仅旋转前臂使手掌平面朝向镜头，观众看见掌心与掌纹面；手指仍大致沿手臂伸出方向，腕部不要过度上折。',
      '禁止做成手指朝上、腕折约 90° 的竖掌「停车/制止」手势；禁止掌心朝上朝天；禁止只露手背。',
      '写明须保留或展示的板块、栏目、步骤、标注方式与版式密度，必须按要求落实；未写明的内容仍默认克制，勿自行扩成信息墙。',
  ...
  if (refs?.hasStyleReference) {
    lines.push(
      '【视觉参考图】只借画风、画法媒介、配色、字体与外貌；禁止把参考图的姿态、手心朝向原样搬进成品。',
      '画风：须对齐画法媒介、配色、字体与整体气质（如线刻/版画/单色解剖图 vs 写实摄影，须与参考一致，禁止改成另一种渲染媒介）；人物肢体与皮肤的画法属于画风，须一并对齐。',
      '人物外貌：主要用于锁定身份、五官、发型、体型、着装气质；若参考为多人分栏，人数与性别一致并全部出镜。',
      '多人肢体须各自独立生成，且性别差异要一眼可辨：男手明显更大更厚、指节骨点更凸、前臂肌肉更鼓；女手明显更小更纤细、腕部更窄、线条更柔；禁止上下栏同一只手克隆、同尺寸同轮廓几乎无差别。',
      requirement
        ? '反约束：勿复刻其原文案、图注、引出线与标记点落点；勿把参考图上的标识点、测量线或标注结构原样搬进成品；标识与标注位置以【我的要求】与【本张画面 / 图文内容】为准，参考图不作定位依据。版式密度以【我的要求】为准，勿因正文或参考图变成长文信息图。'
        : '反约束：勿复刻其原文案、图注、引出线与标记点落点；勿把参考图上的标识点、测量线或标注结构原样搬进成品；标识与标注位置以【本张画面 / 图文内容】为准，参考图不作定位依据；亦勿复刻其高密度信息墙版式。',
    );
  }

  if (requirement) {
    lines.push(
      '姿态优先级复核：【我的要求】＞【本张画面 / 图文内容】＞视觉参考。未写明姿态时，再按正文需要调整。',
      '【本张画面 / 图文内容】仅作事实与文案来源（知识库），非必须全文上屏；按【我的要求】提炼所需标题与文案；未要求铺开时，勿按正文 ## 小节结构整页铺开。未写明姿态时，可据此调整出镜与动作。',
      prompt.trim(),
    );
  } else {
    lines.push(
      '【本张画面 / 图文内容】仅作事实与文案来源，非必须全文上屏：在不失真、不编造的前提下取舍精简，通常保留标题与 1～3 个最能传达主题的要点或一句摘要即可；勿按正文 ## 小节、列表、步骤逐条铺满画面，禁止多栏信息框、密排图标栏与百科式长文信息图。姿态可按正文需要调整，勿被视觉参考姿态锁死。',
      prompt.trim(),
    );
  }
  return lines.join('\n');
