...
export function buildImageTextPrompt(
  ...
  if (refs?.hasStyleReference) {
    if (requirement) {
      lines.push(
        '【视觉参考图】只对齐画风、配色、字体与整体气质；若参考为多人分栏，保留多人同框关系。',
        '反约束：勿复刻其人物相貌、原文案、图注、引出线与标记点落点；勿把参考图上的穴位/部位标识位置、测量线或标注结构原样搬进成品；解剖与标识位置以【我的要求】与【本张画面 / 图文内容】为准，参考图不作定位依据。',
        '版式密度以【我的要求】为准，勿因正文或参考图变成长文信息图。',
      );
    } else {
      lines.push(
        '【视觉参考图】只对齐画风、配色、字体与排版气质；若参考为多人分栏，保留多人同框关系。',
        '反约束：勿复刻其人物相貌、原文案、图注、引出线与标记点落点；勿把参考图上的穴位/部位标识位置、测量线或标注结构原样搬进成品；解剖与标识位置以【本张画面 / 图文内容】为准，参考图不作定位依据；亦勿复刻其高密度信息墙版式。',
      );
    }
  }
