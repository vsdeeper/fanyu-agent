...
export const DETAIL_IMAGE_THEME_DUTIES: Record<DetailImageThemeId, string> = {
  brand:
    '建立品牌第一印象：定位、气质与可识别符号。中景或半身级产品全貌，不要特写局部，不得写成卖点清单或使用现场。背景须用能承载品牌气质的场景、环境或材质铺陈，禁止单调、纯色背景与空洞留白棚拍。品牌 Logo 可当设计元素独立呈现于画面（用户提供了品牌 Logo 时以它为准，否则用产品上已有的品牌标识，不能只让它出现在产品机身上）；商业分析确无品牌信息时不要编造品牌名。',
  sellingPoint:
...
const DETAIL_IMAGE_REWRITE_CARD_BASE_INSTRUCTIONS = `你是电商详情页结构策划。只写当前这一屏的设计目标与展示重点，不定死上图文案。使用中文简体。
...
- 主题卡不要写色板、字体、整套视觉规范或定死的上图短句；Logo 只写「是否作为独立设计元素呈现」，不写具体位置；这些由出图环节统一决定。
