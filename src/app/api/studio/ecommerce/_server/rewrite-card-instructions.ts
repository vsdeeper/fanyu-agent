...
export const DETAIL_IMAGE_THEME_DUTIES: Record<DetailImageThemeId, string> = {
  brand:
    '建立品牌第一印象：定位、气质与可识别符号。中景或半身级产品全貌，不要特写局部，不得写成卖点清单或使用现场。背景须用能承载品牌气质的场景、环境或材质铺陈，禁止单调、纯色背景与空洞留白棚拍。画面文案搭配品牌名、定位短句；品牌 Logo 可当设计元素独立呈现于画面（用户提供了品牌 Logo 时以它为准，否则用产品上已有的品牌标识，不能只让它出现在产品机身上）；商业分析确无品牌信息时才可省略品牌名。',
  sellingPoint:
    '设计目标聚焦本屏主主张；画面文案把该主张收成可上图短句；展示重点须围着同一件事展开。禁止把分析里的卖点清单整段平铺进来。机位须能把该主张看清，可斜侧或微距，不得复用「品牌认知」的定位口号与同一正视全身。',
  detail:
    '材质、工艺、结构或外观特写。画面文案写细节相关短句。必须近景/局部特写，并换一个不同于前两屏的观察角度。不得写成功能机制演示，也不得复用核心卖点的主主张。',
  feature:
    '看见机制如何发生（结构、操作、效果）。画面文案写机制相关短句。主机制不得与「核心卖点」的主主张相同；机位须能看见机制发生（操作角度、出风/结构开口等），不得停在静物正视特写。',
  scene:
    '真实使用现场与情境。画面文案写情境短句。必须拉远，产品按使用比例出现在现场中，不得是静物台或纯特写，不得复用前四屏的信息点。',
  reason:
    '收束购买动机与体验结果。画面文案写利益或安心感短句。不得再罗列功能参数，不得与「使用场景」写成同一现场；产品占比与机位须与前五屏均不同。',
};

/** 各张职责：与主图分析指令中对应节一致，只注入当前张；与详情图同名 themeId 的职责完全不同。 */
export const MAIN_IMAGE_THEME_DUTIES: Record<MainImageThemeId, string> = {
  product:
    '设计目标写第一眼印象；画面文案给品牌名/产品名等短句（商业分析确无品牌信息时可用品类短句）；展示重点注重产品在实际场景中的展示，突出产品格调，禁止单调、纯色背景与空洞留白棚拍；品牌 Logo 可当设计元素融入画面。产品是画面主角，摆拍呈现、不写使用动作，与「使用场景」的现场互斥。',
  sellingPoint:
    '设计目标聚焦本张主主张；画面文案把该主张收成可上图短句；展示重点须围着同一件事展开。禁止把分析里的卖点清单整段平铺进来。拍法须换成能把该主张视觉化的另一空间切片，不得复用产品展示的特写台面。',
  feature:
    '设计目标点出主机制；画面文案写机制相关短句；展示重点写同机制的细节呈现。主机制不得与「核心卖点」的主主张相同。拍法须能看见机制发生（气流、摆动、结构细节），不得复用前两张的空间。',
  scene:
    '设计目标写情境；画面文案写情境短句；展示重点写真实使用现场的取景。拍法必须是真实使用现场，不得是静物台或纯特写。',
  value:
    '设计目标写利益；画面文案写利益短句；展示重点写体验结果与氛围的呈现，不做说明书罗列。拍法与「使用场景」的现场不得相同。',
};

const DETAIL_IMAGE_REWRITE_CARD_BASE_INSTRUCTIONS = `你是电商详情页结构策划。只写当前这一屏的设计目标、画面文案与展示重点。使用中文简体。
...
const MAIN_IMAGE_REWRITE_CARD_BASE_INSTRUCTIONS = `你是电商主图策划。只写当前这一张的设计目标、画面文案与展示重点。使用中文简体。

硬性规则：
- 禁止询问、禁止确认门、禁止请用户补充后再写；资料不足时按已有的商业分析与产品资料合理收束。
- 用户 prompt 含【产品资料】时，品牌、产品名、规格数值等第一手产品事实以【产品资料】为准。
- 禁止调用工具、禁止出图、禁止输出图片链接。
