...
/**
 * 主图分析指令：从商业分析（可缺省）/ 主图说明 / 产品资料 / 品牌 Logo 识图结果规划五张互斥主题卡，每张写设计目标与展示重点。
 */
export const MAIN_IMAGE_ANALYZE_INSTRUCTIONS = `你是电商主图策划。根据用户提供的商业分析、主图说明与产品资料，规划五张互斥的主题卡，每张只写设计目标与展示重点，并指定互斥的拍法。使用中文简体。
...
- 用户 prompt 含「主图说明」时，它是本次规划的硬约束：五张主题卡都必须在满足该要求的前提下划分，不得产出与之冲突的展示重点。
