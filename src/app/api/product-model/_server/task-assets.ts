...
import { productModelTaskAssets } from '@/lib/db/schema';
import type { ProductModelStepKey } from '../_shared/task-types';

type TaskAssetRecord = typeof productModelTaskAssets.$inferSelect;

function getTaskAssetsRoot(): string {
  return path.join(path.resolve(getChatDir()), 'product-model');
}
...
export function buildTaskAssetUrl(taskId: string, assetId: string): string {
  return `/api/product-model/tasks/${encodeURIComponent(taskId)}/assets/${encodeURIComponent(assetId)}`;
}
...
export function getTaskAsset(taskId: string, assetId: string): TaskAssetRecord | undefined {
  return getDb()
    .select()
    .from(productModelTaskAssets)
    .where(and(eq(productModelTaskAssets.taskId, taskId), eq(productModelTaskAssets.id, assetId)))
    .get();
}
