...
- 仅讨论如何画、不请求出图时不要调用
- 面向电商/商品出图（用产品图为底生成主图/详情图/营销图的 i2i）时：先向用户展示拟用方案与完整生图提示词，待用户明确确认（「确认/就用这个出图/开始生成」）后再调用本工具；用户给的类型/数量/平台/画幅属方案输入，不视为出图指令，未经确认不得出图。仅用户明说「直接出图/不用确认」时才可直接出图
...
function createGenerateImageTool(
  ...
  return tool({
    description:
      '根据描述生成或编辑图片。仅在用户明确要求出图或改图时调用；讨论绘画技巧时不要调用；从产品图生成电商商品图（主图/详情图/营销图）时，须在用户确认方案与提示词后再调用。',
