...
import { registerEcommerceImages } from './catalog/register-ecommerce-images';
 ...
 * | acceptEcommerceUploads   | 本轮电商上传图链路开启，register_ecommerce_images 才可见；其它出图 skill 不得登记产品图/参考图 |
 */
function pickVisibleTools(
  ctx: Pick<
    AgentToolContext,
    'mainModelAcceptsImage' | 'providerHasNativeWebSearch' | 'ecommerceUploadsEnabled'
  >,
): AgentToolDefinition[] {
  ...
  const acceptEcommerceUploads = ctx.ecommerceUploadsEnabled === true;
  ...
  return TOOLS.filter(
    (item) =>
      ...
      (acceptEcommerceUploads || !item.requiresEcommerceUploads),
  );
}
