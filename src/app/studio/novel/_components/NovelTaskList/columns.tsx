...
import type { NovelTaskListItem } from '@/app/api/studio/novel/_shared/task-types';
...
export function createTaskColumns({
  ...
  onMaterial: (task: NovelTaskListItem) => void;
  onDelete: (task: NovelTaskListItem) => Promise<void>;
}): TableColumnsType<NovelTaskListItem> {
  return [
    {
      title: '任务名称',
      dataIndex: 'name',
    ...
    },
    {
      ...
      render: (_, task) => (
        <Space size="middle">
          ...
          <Typography.Link onClick={() => onMaterial(task)}>写作</Typography.Link>
          <Popconfirm
