...
import { getTaskEditorPath, normalizeSearchName, requestProductModelTasks } from '../utils';
...
export function useProductModelTaskList() {
  const { message } = App.useApp();
  const router = useRouter();
  const [searchForm] = Form.useForm<SearchFormValues>();
  const [loading, setLoading] = useState(true);
  const [items, setItems] = useState<ProductModelTaskListItem[]>([]);
  const [total, setTotal] = useState(0);
  const [createOpen, setCreateOpen] = useState(false);
  const [editingTask, setEditingTask] = useState<ProductModelTaskListItem>();
  ...
  const fetchList = useCallback(async (next: TaskListQuery) => {
    ...
    setLoading(true);
    try {
      const result = await requestProductModelTasks({
  ...
  useEffect(() => {
    let cancelled = false;
    void requestProductModelTasks(INITIAL_QUERY)
      .then((result) => {
        if (cancelled) return;
        setItems(result.items);
        setTotal(result.total);
  ...
  const onTableChange: TableProps<ProductModelTaskListItem>['onChange'] = (pagination) => {
  ...
  const handleCreateSuccess = (task: ProductModelTaskDetail) => {
  ...
  const onDelete = useCallback(
    async (task: ProductModelTaskListItem) => {
      await apiDelete(`/api/product-model/tasks/${task.id}`);
