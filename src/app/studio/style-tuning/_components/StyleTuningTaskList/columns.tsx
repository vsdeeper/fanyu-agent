...
import type { StyleTuningTaskListItem } from '@/app/api/studio/style-tuning/_shared/task-types';
...
export function createTaskColumns({
  ...
  onMaterial: (task: StyleTuningTaskListItem) => void;
  onDelete: (task: StyleTuningTaskListItem) => Promise<void>;
}): TableColumnsType<StyleTuningTaskListItem> {
  return [
    {
      title: '任务名称',
      dataIndex: 'name',
    ...
    },
    {
      ...
      render: (_, task) => (
        <Space size="middle">
          ...
          <Typography.Link onClick={() => onMaterial(task)}>进入工作台</Typography.Link>
          <Popconfirm
