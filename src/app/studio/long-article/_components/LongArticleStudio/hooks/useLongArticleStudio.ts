...
export function useLongArticleStudio(task: LongArticleTaskDetail) {
  ...
  const [panelForm] = Form.useForm<LongArticlePanelValues>();
  // 左栏初值只算一次：Form 二次挂载是 store 赢（只补缺失键），重算只会白白多渲染
  const [panelInitialValues] = useState<LongArticlePanelValues>(() => ({
  ...
  const panelValues: LongArticlePanelValues = watched ?? panelInitialValues;
  ...
  async function persistResearch() {
    ...
    const saved = await saveLongArticleStep(task.id, 'research', next);
  ...
  async function persistPlan() {
    ...
    const saved = await saveLongArticleStep(task.id, 'plan', next);
  ...
  async function persistDraft(overrides?: {
    ...
    const saved = await saveLongArticleStep(task.id, 'draft', next);
  ...
  async function runSse(
    ...
    try {
      ...
      console.error('[long-article-studio]', url, err);
    ...
    await runSse(
      ...
      async (fullText) => {
        ...
        if (parsed.angles.length === 0) {
          ...
          console.warn('[long-article-studio] research missing angles', {
        ...
        try {
          ...
          console.error('[long-article-studio] persist research', err);
        }
      },
      'researching',
      'research',
    ...
    await runSse(
      ...
      async (fullText) => {
        ...
        if (parsed) {
          ...
          setPhase('planned');
          try {
            const saved = await saveLongArticleStep(task.id, 'plan', nextPlan);
            lastPlanRef.current = saved;
          } catch (err) {
            console.error('[long-article-studio] persist plan', err);
          }
          return;
        }
    ...
    await runSse(
      ...
      async (fullText) => {
        ...
        try {
          ...
          console.error('[long-article-studio] persist draft', err);
        }
      },
      'drafting',
      'draft',
  ...
  async function handlePlanImages() {
    ...
    if (styleReference) {
      try {
        ...
        console.error('[long-article-studio] style reference', err);
    ...
    await runSse(
      ...
      async (fullText) => {
        ...
        try {
          ...
          console.error('[long-article-studio] persist images', err);
        }
      },
      'illustrating',
      'images',
  ...
  async function withWatermark(url: string): Promise<string> {
    ...
    try {
      ...
      console.error('[long-article-studio] compose watermark', err);
    ...
    try {
      ...
      if (nextUrl) {
        try {
          ...
          console.error('[long-article-studio] persist after generate', err);
        }
      }
    } catch (err) {
      if (!isAbortError(err)) {
        console.error('[long-article-studio] generate slot', err);
  ...
  async function handleUploadSlot(slotId: string, file: File) {
    try {
      ...
      try {
        ...
        console.error('[long-article-studio] persist after upload', err);
        message.warning('图片已填入槽位，但保存失败，请稍后重试');
      }
    } catch (err) {
      console.error('[long-article-studio] upload slot', err);
  ...
  async function handleApplyHistory(historyId: string) {
    ...
    try {
      ...
      console.error('[long-article-studio] apply history', err);
    }
  }
  ...
  async function handleRemoveHistory(historyId: string) {
    ...
    try {
      ...
      console.error('[long-article-studio] remove history', err);
  ...
  async function handleCopyArticle() {
    try {
      const payload = buildArticleCopyHtml(markdown);
  ...
  async function handleNext() {
    ...
    try {
      ...
      console.error('[long-article-studio] next', err);
    } finally {
      setNavLoading(false);
    }
  }
   ...
   */
  async function handlePanelFieldChange(changed: Partial<LongArticlePanelValues>) {
    if (!('styleReferenceImages' in changed) && !('watermarkImages' in changed)) return;
    try {
      await persistDraft();
    } catch (err) {
      console.error('[long-article-studio] persist reference images', err);
    }
  }
  ...
  function openSlotDrawer(slotId?: string) {
    ...
    if (nextId) {
      requestAnimationFrame(() => {
        document.getElementById(`long-slot-${nextId}`)?.scrollIntoView({
          block: 'nearest',
          behavior: 'smooth',
        });
      });
