...
import {
  buildArticleCopyHtml,
  markdownToPlainText,
  stripLeadingArticleTitle,
} from './article-copy';

describe('buildArticleCopyHtml', () => {
  it('输出带内联 style 的小节标题、段落与加粗，去掉篇名与配图标注', () => {
    ...
    const { html, plain } = buildArticleCopyHtml(markdown);

    expect(html).toContain(`<section style="${ARTICLE_COPY_STYLES.section}">`);
    expect(html).not.toContain('主标题');
    expect(html).not.toContain('<h1');
    expect(html).toContain(`<h2 style="${ARTICLE_COPY_STYLES.h2}">小节一</h2>`);
    expect(html).toContain(`<p style="${ARTICLE_COPY_STYLES.p}">开篇段落。</p>`);
    expect(html).toContain(`<strong style="${ARTICLE_COPY_STYLES.strong}">重点</strong>`);
    expect(html).toContain(`<em style="${ARTICLE_COPY_STYLES.em}">强调</em>`);
  ...
  it('无一级标题时原样保留正文', () => {
    const { html } = buildArticleCopyHtml('只有正文一段。');
    expect(html).not.toContain('<h1');
    expect(html).toContain('只有正文一段。');
  });

  it('空正文返回空载荷', () => {
    expect(buildArticleCopyHtml('   ')).toEqual({ html: '', plain: '' });
    expect(buildArticleCopyHtml('# 仅有标题')).toEqual({ html: '', plain: '' });
  });
});
