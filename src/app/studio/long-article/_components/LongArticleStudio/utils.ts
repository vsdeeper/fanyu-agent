...
/** 落盘长文步骤快照，返回服务端资产 URL 化后的 data。 */
export async function saveLongArticleStep<T>(
  taskId: string,
  stepKey: LongArticleStepKey,
  data: T,
): Promise<T> {
  const record = await apiPut<LongArticleTaskStepRecord>(
    `/api/studio/long-article/tasks/${encodeURIComponent(taskId)}/steps/${stepKey}`,
    {
      snapshotVersion: LONG_ARTICLE_STEP_SNAPSHOT_VERSION,
      data,
    },
  );
...
/**
 * 复制富文本（平台编辑器认 text/html + 内联 style）。
 ...
 * 长文正文把所有配图按同一栏宽展示，1K / 2K、16:9 / 3:4 混排时页面上水印也就一样大、一样靠角。
 * 固定像素（小图上显大）或按高度取比例（横竖版忽大忽小）都做不到这一点。
 * 水印本身等比缩放进「宽 = 成图宽 × WATERMARK_WIDTH_RATIO」的方形盒子，故竖长水印会按高度回收。
 * 底部留白比右边大，为常见平台底栏留白（见 WATERMARK_BOTTOM_RATIO）。
 */
