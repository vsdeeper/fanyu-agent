...
/** 图文任务列表：查询、新增、改名、删除和进入工作台。 */
export default function ImageTextTaskList() {
  const createColumns = useCallback(
    ({
      ...
      onEdit: (task: ImageTextTaskListItem) => void;
      onOpenEditor: (task: ImageTextTaskListItem) => void;
      onDelete: (task: ImageTextTaskListItem) => Promise<void>;
    }) => createTaskColumns({ onEdit, onMaterial: onOpenEditor, onDelete }),
    [],
  );
  const list = useStudioTaskList<ImageTextTaskListItem, ImageTextTaskDetail>({
    apiBase: '/api/studio/image-text',
  ...
  return (
    <StudioPageShell title={TASK_LIST_TITLE}>
      ...
      >
        <TaskNameFormModal<TaskFormValues, ImageTextTaskListItem, ImageTextTaskDetail>
